...
export const ResultsViewer: React.FC<ResultsViewerProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'summary' | 'risks' | 'text'>('summary')
  ...
  // Sort risk assessments
  const sortedRisks = [...results.riskAssessments].sort((a, b) => {
    switch (sortBy) {
      case 'risk':
  ...
  return (
    ...
    >
      ...
      <div className="border-b p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              {results.session.contentLength.toLocaleString()} characters analyzed
            </p>
          </div>
          <div className="text-right">
            <div className={cn('text-2xl font-bold', getRiskLevelColor(results.session.riskLevel))}>
              {results.session.riskScore}% Risk
            </div>
            <div className="text-sm text-gray-600">
              Processed in {formatDuration(results.session.processingTimeMs)}
            </div>
          </div>
        </div>
        ...
        {/* Overall Confidence */}
        <ConfidenceIndicator
          score={results.session.confidenceScore}
          size="lg"
          className="mb-4"
        />
        ...
        <div className="flex space-x-1 bg-gray-100 rounded-lg p-1">
          {[
            ...
            { id: 'risks', label: 'Risk Details', count: results.riskAssessments.length },
      ...
      <div className="border-t p-4 bg-gray-50 rounded-b-lg">
        <div className="flex items-center justify-between text-sm text-gray-600">
          <div>
            Analysis completed on {new Date(results.session.completedAt || results.session.createdAt).toLocaleDateString()}
          </div>
          <div>
            Results expire on {new Date(results.session.expiresAt).toLocaleDateString()}
          </div>
        </div>
      </div>
    </div>
